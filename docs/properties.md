...
| 32 | The installable wheel contains the runtime package only. Tests and the banc ship in the sdist, not in the wheel. | `scripts/verify_artifacts.py`; `check-wheel-contents` in CI; review of `MANIFEST.in` (no prune of `tests/`) |
| 34 | The test suite shipped in the sdist runs to completion on an unpacked sdist, outside any git repository. Collection produces 0 error; the run produces 0 failed. | `scripts/verify_artifacts.py` (sdist ships `scripts/` and `fixtures/`; the wheel ships neither); `tests/test_distribution_artifacts.py` (`test_release_artifact_inspector_rejects_sdist_missing_scripts`, `test_release_artifact_inspector_rejects_sdist_missing_fixtures`, `test_release_artifact_inspector_rejects_wheel_containing_scripts`, `test_release_artifact_inspector_rejects_wheel_containing_fixtures`); CI job `artifacts` step that unpacks the sdist and runs `pytest -q`; modules that need git or `.github/` skip when those resources are absent (`tests/test_draft_release.py`, `tests/test_release_whitespace.py`, `tests/test_tracked_gitignored.py`) |
