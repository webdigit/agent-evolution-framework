...
| 36 | Locally produced distribution artifacts are ignored, including `dist-<suffix>` trees at the repository root. `distribution/` and `distinct/` are not ignored. A `dist-*` tree under a subdirectory is not ignored: `/dist-*/` is root-anchored on purpose, and must not be widened to `dist*/`. | `tests/test_tracked_gitignored.py` (`test_local_distribution_artifact_ignore_rule`) |
